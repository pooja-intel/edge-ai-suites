...
## Old Platform Compatibillity

The configuration in the Docker script supports MTL/ARL platform by default.
To run on old platforms like ADL/RPL, please remove the environment variable `-e DISPLAY_NEW_PLATFORM=1` in `run.sh` and `DISPLAY_NEW_PLATFORM: "1"` in `docker-compose.yml`.
