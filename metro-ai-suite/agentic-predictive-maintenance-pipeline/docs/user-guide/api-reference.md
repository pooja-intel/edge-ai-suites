...
The APM blueprint exposes REST APIs through two services. Both services are accessible via the Nginx reverse proxy at `http://localhost:8080`.
...
| Method | Path | Description |
|--------|------|-------------|
| `POST` | `/runs` | Trigger a new agent pipeline run (async) |
...
Metrics from both services are collected by the metrics-manager service and available at:

- **Storage service**: `http://localhost:9091/metrics` — exposes `apm_storage_detections_total`
- **Agent service**: `http://localhost:9091/metrics` — exposes `apm_agent_runs_total` and `apm_agent_runs_completed`
