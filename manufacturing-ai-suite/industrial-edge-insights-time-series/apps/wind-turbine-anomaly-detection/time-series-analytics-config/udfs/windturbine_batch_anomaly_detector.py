            ...
            if i in valid_index_set:
                ...
                if check_for_anomalies:
                    ...
                    if sum(self.last_states) == self.n_steps:
                        arr = np.array(self.last_anomalies)
                        x_feat = arr[:, 0].reshape(-1, 1)
                        y_feat = arr[:, 1]

                        if intel_scikitlearn_extension == 'true' and self.device != 'cpu':
                            with config_context(target_offload=self.device,
                                                allow_fallback_to_host=True):
                                self.lm.fit(x_feat, y_feat)
                        else:
                            self.lm.fit(x_feat, y_feat)

                        if abs(self.lm.coef_[0]) < 200:
